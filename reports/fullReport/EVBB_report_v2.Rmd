 ...
 * measurement of observations only occurs when the car is switched on and/or plugged in and charging. As a result no data will be collected when the EV is switched off. This means that there are large non-erroneous 'gaps' in the data which represent 'no charging' (and also 'no driving'). We have included these as '0 power demand' in the analyses by imputing missing time 1 minute time stamps for each vehicle between their first and last known observations. This means we are now able analyse power demand profiles for _all_ vehicles, and not just those that were charging;
 ...
 * that these vehicles are driven by "early adopters" who have opted to install the measuring devices in order to collect their vehicle usage data. As a result the data may not be representative of the usage patterns of current or future EV drivers [@rezvani_advances_2015,@li_review_2017].
