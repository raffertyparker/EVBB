...
p <- ggplot2::ggplot(evPlotDT[!(chargeType %like% "Not")], aes(x = qHour, y = sampleCarkW, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Charge Type") +
  labs(y = "Mean kW per quarter hour", x = "Time of day") +
  facet_grid(weekdays ~ .)
...
Figure \@ref(fig:pcCharging) showed the mean % of EVs which were charging in each quarter-hour of the day by charge type. Based on Table \@ref(tab:obsPower), let us assume:
