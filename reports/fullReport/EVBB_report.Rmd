...
rawCap <- paste0("Data source: Flip The Fleet (original data)",
                 "\n Period: ", min(rawDT$date, na.rm = TRUE), " - ", max(rawDT$date, na.rm = TRUE))
...
The original supplied data consisted of `r tidyNum(nrow(rawDT))` observations for the period `r startDate` to `r endDate`.
...
This left `r data.table::uniqueN(cleanDT$dvID)` remaining vehicles, and `r tidyNum(nrow(cleanDT))` observations as shown in Table \@ref(tab:finalTable).
...
# for use at start of localised myCaps
...
kableExtra::kable(t, digits = 2, caption = "Descriptive statistics for charging (kW)") %>%
  kable_styling()
