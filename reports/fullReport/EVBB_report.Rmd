...
Based on a relatively small sample of `r data.table::uniqueN(dt$dvID)` domestic electric vehicles provided by our research partner [FlipTheFleet](https://flipthefleet.org/) and monitored over 8 months from `r format(min(df$date), format = "%B %Y")` to `r format(max(df$date), format = "%B %Y")`. The recorder provided measurements at 1 minute frequency of charging power and battery charge state.
