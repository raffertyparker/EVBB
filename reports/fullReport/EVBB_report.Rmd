...
cleanDT[, peakPeriod := ifelse(startTime >= pmPeakStart & 
                                ...
                                "Evening peak",
                                peakPeriod)]
cleanDT[, peakPeriod := ifelse(startTime > pmPeakEnd,
                                "Late evening",
                                peakPeriod)]
# re-order factor so Standard plots to the left (makes more sense)
cleanDT[, peakPeriod := forcats::fct_relevel(peakPeriod, 
                                             "Early morning",
                                             "Morning peak",
                                             "Day time",
                                             "Evening peak",
...
cleanDT[, .(nObs = .N), keyby = .(hour = lubridate::hour(r_dateTime), peakPeriod)]
```
...
> The analysis reported below is work in progress. Please check with the authors before quoting or re-using (with attribution) results/plots. Reporting interim results can seriously damage your health. #YMMV.
...
The lack of data from EVs which are neither charging nor being driven means that it is impossible to directly estimate the 'average' power demand for the sample. Instead we use an estimate of the proportion of EVs which were charging and our derived median 'standard' and 'rapid' kW demand to estimate these values.
...
We can now calculate the mean kW demand per EV per time period by multiplying these values by the appropriate % of EVs who were charging by charge type. The results of doing so are shown in \@ref(fig:setPower). 
...
In addition to electricity demand (power in kW) we are also interested in overall energy consumption (kWh). To do this we take the mean kW per quarter hour per EV values reported in \@ref(fig:setPower) and divide them by 4 to give mean kWh per quarter hour per EV.
...
Table \@ref(tab:estimatekWhByPeak) and Figure \@ref(tab:estimatekWhByPeak) repeats this analysis but shows the results by peak period.
...
ggplot2::ggsave(paste0(pPath,"/SoCplot2.png"), p ) # for PCE report
...
Figure \@ref(fig:SoCplotBegin) repeats this analysis but uses the cleaned and corrected inferred start/end of charging sequence data instead of all charging observations. Figure \@ref(fig:SoCplot3) shows very similar distributions to the previous 'all-observations' plot (Figure \@ref(fig:SoCplot2corrected)) and confirms that sequences of standard charging in particular most frequently start with battery state of charge over 50%. 
...
ggplot2::ggsave(paste0(pPath,"/SoCplot2corrected.png"), p ) # for PCE report
...
plotDT[, cut := ifelse(SoC_percent <= 5, "<= 5%", "Other")]
plotDT[, cut := ifelse(SoC_percent > 50, "> 50%",cut)]
t <- plotDT[, .(nObs = .N), keyby = .(cut, chargeType)]
...
We can now use the results shown in Figure \@ref(fig:pcCharging) to construct a any number of scenarios such as:
...
To determine whether this is a significant proportion of New Zealand national demand we convert the mean the MW values to MWh per half-hour for comparison with mean MWh per half-hour national generation values for the same period sourced from the EA's EMI XX reference XXX.
...
#### Summer
Figure \@ref(fig:convertEVtoMWh) shows the result of this calculation for the study period. Note that this was during the summer so evneing peaks are lower.

```{r convertEVtoMWh, fig.cap("Actual wholesale generation and estimated energy consumption under Scenario 1 for the study period.")}
...
p <- ggplot2::ggplot(dt[!(chargeType %like% "Not")], aes(x = halfHour, y = kWh/1000, colour = chargeType)) +
  ...
  labs(x = "Time of Day", y = "MWh per half hour") +
  facet_grid(weekdays ~ .)
...
XX need to do a % add plot & check calculations XX
