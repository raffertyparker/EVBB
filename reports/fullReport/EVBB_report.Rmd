...
### Method 2: Half-hourly aggregation
...
Here we take the floor(1 min) data described in XX and:
...
```{r makeHalfHourlyKWh}
minDT[, r_dateTime30Min := lubridate::floor_date(r_dateTime1MinFl, "30 mins")]

halfHourDT <- minDT[, .(sumkWObs = sum(meankW), 
                  nObs = .N, 
                  nMins = 30), 
              keyby = .(dvID, r_dateTime30Min, chargeType, peakPeriod, weekdays)]

halfHourDT[, meankW := sumkWObs/nMins] # as we would expect 30 obs in a half hour

halfHourDT[, kWh := meankW/2] # as they are half-hours

halfHourDT[, hms := hms::as_hms(r_dateTime30Min)]
...
# sum kwH per day per EV within categories we care about
t <- halfHourDT[!(chargeType %like% "Not"), .(sumkWh = sum(kWh)), keyby = .(chargeType, peakPeriod)]

ggplot(t, aes( x = peakPeriod, y = sumkWh, fill = chargeType)) + 
  geom_col() 
```
