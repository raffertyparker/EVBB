...
sampleStart <- "2018-10-01"
sampleEnd <- "2019-02-28"

# no more processing of cleanDT after this ----
cleanDT <- cleanAllDT[as.Date(r_dateTime) >= as.Date(sampleStart) & as.Date(r_dateTime) < as.Date(sampleEnd)]
 ...
 * all observations collected before `r sampleStart` and after `r sampleEnd` in order to focus analysis on the periods with most EVs present in the data. It is hoped that this will reduce the extent to which the charging behaviour of a small number of EV owners will skew the aggregated results.
...
halfHourDT[, meankW := sumkWObs/nMins] # as we would expect 30 obs in a half hour
halfHourDT[, hms := hms::as.hms(r_dateTime30MinFl)]
plotDT <- halfHourDT[, .(meankW = mean(meankW),
                         sumkW = sum(meankW)), 
                     keyby = .(chargeType, peakPeriod, weekdays, hms)]

ggplot2::ggplot(plotDT, aes(x = hms, y = meankW, colour = chargeType)) +
  geom_point() +
  facet_grid(weekdays ~ .) +
  labs(x = "Time of Day", y = "Mean kw in the half hour",
       caption = "Test plot: all 'half-hour' observations for all EVs")
```
...
# plot for sense
p <- ggplot(halfHourDT, aes(x = hms, y = kWh, colour = chargeType)) +
  geom_point() +
...
Table \@ref(tab:plotKWh) and Figure \@ref(tab:plotKWh) summarise the energy consumption across peak periods without weekday disaggregation. This shows that overall (38.6 + 15.2) = 54% of the energy drawn was overnight off-peak standard charging. Day time standard charging was 19% of the total and day-time rapid only 8%. Evenng peak standard charging was 12% of total energy.
...
Based on Table \@ref(tab:obsPower), we can repeat the process used in Section \@ref(estimatekWh) by estimating:
