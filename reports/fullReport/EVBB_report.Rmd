...
yMin <- min(evPlotDT$nDaily, na.rm = TRUE)
yMax <- max(evPlotDT$nDaily, na.rm = TRUE)
p <- addPeaks(p)
...
We can now calculate the mean kW demand per EV per time period by multiplying these values by the appropriate % of EVs who were charging by charge type. The results of doing so are shown in \@ref(fig:setPower). We can now see that despite the relative rarity of rapid charging, their much higher power demand produces... XX finish XX

```{r setPower}
evPlotDT[, sampleCarkW := ifelse(chargeType == "Standard charging",
                           meanPC * medianStd,
                           NA)
         ]
evPlotDT[, sampleCarkW := ifelse(chargeType == "Rapid charging",
                           meanPC * medianRap,
                           sampleCarkW)
         ]

p <- ggplot2::ggplot(evPlotDT[!(chargeType %like% "Not")], aes(x = qHour, y = sampleCarkW, colour = chargeType)) +
  geom_point() +
  scale_color_discrete(name = "Charge Type") +
  labs(y = "Estimated mean kW per EV per quarter hour", x = "Time of day") +
  facet_grid(weekdays ~ .)

yMin <- min(evPlotDT$sampleCarkW, na.rm = TRUE)
yMax <- max(evPlotDT$sampleCarkW, na.rm = TRUE)
p <- addPeaks(p)
