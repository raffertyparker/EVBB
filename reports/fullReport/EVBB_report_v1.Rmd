...
Figure \@ref(fig:checkStdSequences-kw) plots the first and last charge observation in a sequence for all pairs and for all vehicles where events were classified as (fixed) 'standard' charges. The y value is charging rate (kW) at the start and end of the sequence. Colour (red end of the scale) is used to highlight pairs which show an 'odd' pattern - e.g. the charge rate increased.

```{r checkStdSequences-kw}
...
dt <- firstLastDT[chargeTypeFixed %like% "Standard" & 
                  ...
                  .(startTime, endTime, endTimeTrunc, chargeTypeFixed,startChargekW, endChargekW,
...
p <- p + 
  labs(y = "Charging rate (kW)",
       caption = "Standard charging (fixed) \n Pairs spanning midnight excluded") +
...
Figure \@ref(fig:checkStdSequences-soc) uses the same approach but in this case the y value is charging rate (kW) at the start and end of the sequence. Colour (red end of the scale) is used to highlight pairs which show an 'odd' pattern - e.g. the battery state of charge decreased.

```{r checkStdSequences-soc}
...
Figure \@ref(fig:checkFastSequences-kw) and Figure \@ref(fig:checkFastSequences-soc) repeat these plots but for (fixed) 'fast' charge events.

```{r checkFastSequences-kw}
dt <- firstLastDT[chargeTypeFixed %like% "Fast" & 
...
`chargeType` is used to classify charging events into standard vs fast using the 7 kW threshold. But there may be misclassfications where a sequence starts on a fast charger but power demand declines below the threshold. We can check this and have fixed it in some sections above using the start/end pairs.
