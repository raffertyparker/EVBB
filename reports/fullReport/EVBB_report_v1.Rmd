...
# default fixed ----
firstLastDT <- firstLastDT[, chargeTypeFixed := chargeType]

# If any of them are fast, change
firstLastDT <- firstLastDT[, chargeTypeFixed := ifelse(chargeTypeError == "Error: first = Standard, last = Fast" |
                                                         chargeTypeError == "Error: first = Fast, last = Standard",
                                                       "Fast charging",
                                                       chargeType)]

firstLastDT <- firstLastDT[, chargeTypeFixed := factor(chargeTypeFixed, levels = c('Not charging', 
                                                  'Standard charging', 
                                                  'Fast charging'))] # auto-sets the order

# test
with(firstLastDT[chargeFlag %like% "First"], table(chargeTypeFixed, chargeType, useNA = "always"))
...
ggplot2::ggplot(plotDT, 
  ...
  facet_wrap(chargeTypeFixed ~ ., scales = "free") +
    labs(x = "Minutes", 
       caption = genCap)
```
...
ggplot2::ggplot(plotDT, 
  ...
  facet_grid(chargeTypeFixed ~ ., scales = "free") +
  labs(x = "Minutes", 
       caption = genCap
       )
```
 ...
 * duration < 8 minutes for standard charging (`r nrow(firstLastDT[pairDuration < 8 & chargeTypeFixed %like% "Standard"])` observations - noting that some of these may be short low power 'fast charge' events as discussed in Section \@ref(#chargeType))
 * duration > 840 minutes for fast charging (`r nrow(firstLastDT[pairDuration > 840 & chargeTypeFixed %like% "Fast"])` observations)
...
It has been suggested that EV charging is more likely to occur in the early evening [@langbroek_when_2017]. Figure \@ref(fig:chargeBeginsTime) uses a density plot to represent the proportion of charging and non-charging observations at different times of day be weekday vs weekends. The plot clearly shows non-charging during day-time use and also shows a bi-model distribution for fast charging (non-corrected categorisation). Standard charging also shows a bi-modal distribution with a peak around 22:00 on weekdays and another at 01:00 presumably indicating the use of timed or 'smart' charging or trickle events.
...
p <- ggplot(plotDT, aes(x = as.POSIXct(qHour), fill = chargeTypeFixed)) + # we have to force as.POSIXct if we want to use date_labels
  geom_density(alpha = 0.3) +
  theme(legend.position = "bottom") +
  facet_grid(weekdays ~ .)
...
It seems likely that the 'standard charge' day-time peak is skewed by mis-classified short low power 'fast charge' observations (see Section \@ref(#chargeType)). Figure \@ref(fig:medianChargeByTime) attempts to allow for this misclassification by plotting the median rather than the mean. The plot more clearly shows the 10:00 weekday spike which, if we assume that the mis-classified 'fast charges' will be skewing the standard charge mean value upwards, is likely to be due to mis-classified 'fast charging'. However the 18:00 peak persists as does the 14:00 weekend peak while overnight charging levels are relatively stable as we would expect from \@ref(fig:meanChargeByTimeStd).  
...
t <- firstCleanDT[date >= as.Date("2018-09-01"), .(mean = mean(pairDuration),
               ...
               sd = round(sd(pairDuration),2)), keyby = .(chargeTypeFixed)]
...
plotDT <- dataDT[,
                      .(meanDuration = mean(pairDuration),
                        nEVs = uniqueN(dvID)),
                      keyby = .(qHour, chargeTypeFixed, weekdays)]
...
genCap <- paste0("Data source: Flip The Fleet",                  "First charging observations in a sequence, corrected charge type",
                 "\n n EVs: ", uniqueN(dataDT$dvID),
                 "\n n observations: ", nrow(dataDT),
                 "\n Period: ", min(dataDT$date), " - ", max(dataDT$date),
                 "\n Peak periods shaded"
...
p <- ggplot2::ggplot(plotDT, 
                aes(x = qHour, y = meanDuration, colour = chargeTypeFixed, alpha = nEVs)) +
...
# use fixed first in sequence data

dataDT <- cleanDT[chargeFlag %like% "First"]
  
genCap <- paste0("Data source: Flip The Fleet",                  "First charging observations in a sequence",
                 "\n n EVs: ", uniqueN(dataDT$dvID),
                 "\n n observations: ", nrow(dataDT),
                 "\n Period: ", min(dataDT$date), " - ", max(dataDT$date),
                 "\n Peak periods shaded"
...
As can be seen in Figure \@ref(fig:SoCplot1), using the cleaned complete observations data, the majority of standard charges begin while the state of charge is above 90%. This is most likely due to the manner in which the charger regularly turns off and on again near the end of the charging cycle as described in Section \@ref(cleaning).
...
```{r SoCplot2, fig.cap= "Value of state of charge at beginning of charge (values > 90% removed)"}
plotDT <- cleanDT[chargeFlag %like% "First" & SoC_percent < 90]

genCap <- paste0("Data source: Flip The Fleet",                  "First charging observations in a sequence, state of charge < 90%",
                 "\n n EVs: ", uniqueN(plotDT$dvID),
                 "\n n observations: ", nrow(plotDT),
                 "\n Period: ", min(plotDT$date), " - ", max(plotDT$date),
                 "\n Peak periods shaded"
)
...
p + labs(x = "State of charge when charging begins (%)", caption = genCap) +
  facet_grid(chargeType ~ ., scales = "free")
ggsave("plots/SOC_when_charging_begins_cleanDT.png")
```

```{r SoCplot3, fig.cap= "Value of state of charge at beginning of charge (values > 90% removed)"}
plotDT <- firstCleanDT[SoC_percent < 90]

genCap <- paste0("Data source: Flip The Fleet",
                 "\n First charging observations in cleaned sequences, state of charge < 90%",
                 "\n n EVs: ", uniqueN(plotDT$dvID), " n observations: ", nrow(plotDT),
                 "\n Period: ", min(plotDT$date), " - ", max(plotDT$date),
                 "\n Peak periods shaded"
)
...
p + labs(x = "State of charge when charging begins (%)", caption = genCap) +
  facet_grid(chargeType ~ ., scales = "free")
ggsave("plots/SOC_when_charging_begins_firstCleanDT.png")
```

Figure \@ref(fig:SoCplot3) repeats this but using the cleaned and corrected inferred start/end of charging sequence data. These show very similar disatirbutions to the previous plot (Figure \@ref(fig:SoCplot2)) which used all observations and conforms that sequences of stabdnard chargin in particualr most frequently start with battery state of charge over 50%. 

```{r SoCplotTiming, fig.cap= "Mean state of charge at beginning of charge (values > 90% removed)"}

dataDT <- firstCleanDT[SoC_percent < 90]
plotDT <- dataDT[, .(meanChargePC = mean(SoC_percent),
                 nEVs = uniqueN(dvID)), keyby = .(qHour, weekdays, chargeType)]
...
meanDurationStd <- mean(firstCleanDT[chargeTypeFixed %like% "Standard"]$pairDuration, na.rm = TRUE)
medianDurationStd <- median(firstCleanDT[chargeTypeFixed %like% "Standard"]$pairDuration, na.rm = TRUE)

meanDurationFast <- mean(firstCleanDT[chargeTypeFixed %like% "Fast"]$pairDuration, na.rm = TRUE)
medianDurationFast <- median(firstCleanDT[chargeTypeFixed %like% "Fast"]$pairDuration, na.rm = TRUE)

# use clean data (charge type not fixed)
...
```{r checkChargeType}
# Check chargeFlag ----
message("chargeType is used to classify charging events into standard vs fast using the 7 kW threshold. But there may be misclassfications:")

table(firstLastDT$chargeType, firstLastDT$chargeTypeError, useNA = "always")
...
# Create previous/next dateTime var for error checks ----
dt <- dt[, dateTimeBefore := shift(dateTime, type = "lag"), by = id]
dt <- dt[, dateTimeAfter := shift(dateTime, type = "lead"), by = id]

t <- head(dt[is.na(chargeFlag)])

# should = n EVs
message("N observations where previous dateTime unknown (should match to n EVs)")
nrow(dt[is.na(dateTimeBefore)])
message("N observations where next dateTime unknown (should match to n EVs)")
nrow(dt[is.na(dateTimeAfter)])
message("N observations where chargeFlag is unknown")
nrow(dt[is.na(chargeFlag)])
message("These seem to occur when charging is detected but the dateTime before/after is unkown due to data truncation")
```
...
table(dt$chargeFlag, dt$chargeType, useNA = "always")
