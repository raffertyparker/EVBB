...
```{r checkStdSequenceskW, fig.height=10}
...
```{r checkStdSequencesSoC, fig.height=10}
...
Figure \@ref(fig:checkFastSequenceskW) and Figure \@ref(fig:checkFastSequences-soc) repeat these plots but for (corrected) 'fast' charge events.

```{r checkFastSequenceskW, fig.height=10}
...
```{r checkFastSequencesSoC, fig.height=10}
#dt <- dt[, SoC_pcDelta := SoC_pcDelta * -1] # invert so big drops become red in plot
p <- makeSeqChargePlot(dt, y = "startSoC_pc", 
                       yend = "endSoC_pc", 
                       colour =  "SoC_pcDelta")
