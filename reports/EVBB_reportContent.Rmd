...
# Use drake to re-add data to use ----
rawDT <- drake::readd(rawData)
cleanDT <- drake::readd(cleanData)
...
# decision: do not use a 120 min threshold ----
cleanDT <- clean2DT
...
# default corrected ----
firstLastDT[, chargeTypeCorrected := chargeType]

# If either first or last is Rapid, change
firstLastDT[chargeTypeError == "Error: first = Standard, last = Rapid" |
...
firstLastDT[, pairDuration :=  difftime(shift(dateTime, type = "lead"), dateTime, units='mins'), by = id] # set all within id
firstLastDT[chargeFlag %like% "Last", pairDuration := NA] # removes for last

# check the data.tables we think we just created ----
checkDT <- function(l){
  for(o in l){
    print(paste0("Checking: ", eval(o)))
    print(paste0("N rows: ", nrow(get(o))))
    print(paste0("Variables: ", list(names(get(o)))))
  }
}

l <- c("cleanDT", "clean1DT","clean2DT","firstLastDT")
 ...
 * observations were only collected when the car was switched on and/or plugged in and charging. As a result no observations exist for periods when the EV is switched off and so there are large non-erroneous 'gaps' in the data which represent 'no charging' but which are not included as '0 power demand' in the analyses. If the input data file (`r dataFile`) has been `expanded` then we have imputed a very large number of missing timestamps for each vehicle. In this case aggregate power demand statistics will include all observations whether charging or not (charging = 0). If not then aggregate power demand statistics will only include observations for vehicles that were known to be charging or moving (charging = 0), _not for all vehicles in all time periods_;
...
# set which to include (or were included) ----
summaryDT <- rawDT[, .(mean = mean(charge_power_kw, na.rm = TRUE), 
...
# returns no errors with imputed data (need to check if this is correct)
t <- with(firstLastDT, table(chargeTypeError, chargeType, useNA = "always"))
...
Overall 75% of standard charging observations were `r round(stdQT[[2]],2)` kW or more but the figure was `r round(fastQT[[2]],2)` kW or more for rapid charging.
...
genCap <- paste0("Data source: Flip The Fleet",
                 "\n All observations which were not 'fast' charges",
                 "\n n EVs: ", uniqueN(cleanDT$dvID), ", n observations: ", 
                 prettyNum(nrow(cleanDT), big.mark = ","),
                        "\n Period: ", min(cleanDT$date), " - ", max(cleanDT$date)
)
...
```{r meanChargeByTimeStd, fig.cap="Mean charging power demand (kW) by time of day ('fast' charging excluded)"}

dt <- cleanDT[!(chargeType %like% "Fast")]
genCap <- paste0("Data source: Flip The Fleet",
...
p <- ggplot2::ggplot(dt, aes(x = qHour, y = meankW, colour = weekdays, alpha = nEVs)) +
  geom_point() +
  scale_colour_manual(values=cbbPalette) + # use colour-blind friendly palette
  #facet_grid(chargeType ~ ., scales = "free") + # 
  labs(x = "Time of day",
