...
```{r fastcharge only}
ggplot(df %>% filter(charging_rate == "Fast"), aes(x = halfHour, y = charge_power_kw)) + 
  facet_wrap(~weekday) + 
  guides(colour = guide_legend(title = "Vehicle:")) +
  scale_colour_manual(values=cbPalette) + # use colour-blind friendly palette
  geom_histogram() +
...
```
```{r fast/slow charge relative frequencies}
ggplot(df %>% filter(charging_rate == "Slow") , aes(x = halfHour, y = charging_rate)) + 
  facet_wrap(~weekday) + 
  theme(axis.text.x = element_text(angle = 90)) +
  geom_bar(stat = "identity") +
  labs(x = "Time of day (hours)", y = "Frequency of slow charging events")
  ggsave("~/EVBB/plots/frequency_of_slow_charging.png")
  
ggplot(df %>% filter(charging_rate == "Fast") , aes(x = halfHour, y = charging_rate)) + 
  facet_wrap(~weekday) + 
  theme(axis.text.x = element_text(angle = 90)) +
  geom_bar(stat = "identity") +
  labs(x = "Time of day (hours)", y = "Frequency of fast charging events")
