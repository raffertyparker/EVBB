...
The data used consisted of `r nrow(dt)` data points from `r  length(levels(as.factor(df$dvID)))` vehicles over 8 months (April 2018 - January 2019). The recorder provided measurements at 1 minute frequency of charging behaviour and battery charge state.

Due to privacy considerations, the data is not publically available.

## Initial cleaning

There were `r nrow(summaryDT) - nrow(includeDT)` vehicles that had no recorded charging occur. These were immediately discarded from the data.
...
firstLastDT <- firstLastDT[!(firstLastDT$pairDurationFinal > 600 & firstLastDT$chargeType == "Fast charging"),]
...
All further duration-related analysis is conducted with these excessively long or short events removed from the data.
