...
dt <- dt[, chargeFlag := ifelse(shift(charge_power_kw) == 0 & charge_power_kw > 0,
                                "first", "Not charging"), by = id] # the previous value was 0 but this value > 0
dt <- dt[, chargeFlag := ifelse(shift(charge_power_kw == 0, type = "lead") & charge_power_kw > 0, 
                                "last", chargeFlag), by = id] # next value = 0, this value = > 0
dt <- dt[, chargeFlag := ifelse(shift(charge_power_kw) > 0 & charge_power_kw > 0, 
                                "charging", chargeFlag), by = id] # previous value > 0, this value > 0
...
chargeBegins <- filter(df, (shift(charge_power_kw == 0) & charge_power_kw > 0))

#                                "first", chargeFlag), by = id
#chargeBegins <- filter(chargeBegins, SoC_percent < 97)
chargeBegins$chargeFlag <- "Charge begins"
...
chargeEnds <- filter(df, shift(charge_power_kw == 0, type = "lead") & charge_power_kw > 0)
chargeEnds$chargeFlag <- "Charge ends"
...
```{r chargeDT constructed}
chargeDT <- rbind(chargeBegins, chargeEnds)
chargeDT <- arrange(chargeDT, dateTime, id)
#chargeDT <- chargeDT[ , "charge_power_kw" > 1]


# chargingDT <- filter(chargingDT, abs(SoC_percent - shift(SoC_percent)) > 1 )
```
# Note
...
ggplot2::ggplot(firstLastDT[pairOK == "Pair end" & pairDurationFinal > 8], 
                aes(x = pairDurationFinal)) +
...
ggplot(firstLastDT, aes(chargeFlag)) +
...
ggplot(chargeBegins,aes(x=fractime, y= forcats::fct_rev(day_of_week))) +
